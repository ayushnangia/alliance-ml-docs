...
Misuse of volatile might arise because the Java programming language uses the volatile keyword as well. Java's volatile has a totally different meaning from C's volatile. Specifically, Java's volatile keyword in C corresponds to using atomic\_\* (i.e., where '\*' corresponds to a fundamental type name such as int).
