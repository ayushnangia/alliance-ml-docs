...
We provide pre-compiled Python Wheels for PyRETIS in our [Wheelhouse](Wheels.md) that are compatible with different versions of Python and can be installed within a [virtual Python environment](Python.md#Creating_and_using_a_virtual_environment).
...
A Python virtualenv with PyRETIS can be created by running the following series of commands (lines beginning with # are comments, $ is a prompt and (env\_PyRETIS) $ is a prompt with an activated virtualenv):
