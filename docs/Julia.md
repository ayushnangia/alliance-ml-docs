...
If we were to omit the `Libdl.DL_LOAD_PATH` line from the above example, it would happen to work on Graham because Graham has HDF5 installed system-wide. It would fail on Cedar because Cedar does not. The best practice on *any* of our systems, though, is that shown above: Load the appropriate [module](Utiliser_des_modules.md) first, and use the environment variable defined by the module (`HDF5_DIR` in this example) to extend `Libdl.DL_LOAD_PATH`. This will work uniformly on all systems.
...
[![warning](/mediawiki/images/c/c2/Warning_sign_16px.png)](FileWarning_sign_16pxpng.md) Warning
...
We strongly advise against the default PyCall.jl behaviour, which is to use a Miniconda distribution inside your Julia environment. Anaconda and similar distributions [are not suitable on our clusters](Conda.md).
...
starts one main Julia process and 100 worker processes on the nodes specified in the hostfile and runs the program pi\_p.jl in parallel.
...
You can restrict the number of threads Julia can use by setting JULIA\_NUM\_THREADS=k, for example a single process on a 12 cpus-per-task job could use k=12.
...
where version\_of\_cuda is 12.6 if cuda/12.6 is loaded.
