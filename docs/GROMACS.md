...
- English
- [français](GROMACS_fr.md)

# General

[GROMACS](http://www.gromacs.org/) is a versatile package to perform molecular dynamics for systems with hundreds to millions of particles.
...
StdEnv/2023StdEnv/2020StdEnv/2018.3StdEnv/2016.4
...
This [software environment](Standard_software_environments.md) is no longer supported.
...
- [![warning](/mediawiki/images/c/c2/Warning_sign_16px.png)](FileWarning_sign_16pxpng.md) GROMACS versions 2020.0 up to and including 2021.5 contain a bug when used on GPUs of Volta or newer generations (i.e. V100, T4, A100, and H100) with `mdrun` option `-update gpu` that could have perturbed the virial calculation and, in turn, led to incorrect pressure coupling. The GROMACS developers state in the 2021.6 Release Notes:[1]
...
- Version 2020.4 and newer have been compiled for the new [Standard software environment](Standard_software_environments.md) `StdEnv/2020`.
...
For more information on environment modules, please refer to the [Using modules](Utiliser_des_modules.md) page.
...
Please refer to [GROMACS 5.0 Tool Changes](http://www.gromacs.org/Documentation/How-tos/Tool_Changes_for_5.0) and the [GROMACS documentation manuals](http://manual.gromacs.org/documentation/) for your version.
...
Please refer to the page [Running jobs](Scheduler.md) for help on using the SLURM workload manager.

### Serial jobs

Here's a simple job script for serial mdrun:

**File :** serial\_gromacs\_job.sh
...
NarvalRorqualFirNibiTrillium

**File :** gromacs\_whole\_node\_narval.sh
...
**File :** gromacs\_whole\_node\_rorqual.sh
...
**File :** gromacs\_whole\_node\_fir.sh
...
**File :** gromacs\_whole\_node\_nibi.sh
...
**File :** gromacs\_whole\_node\_trillium.sh
...
Please read [Using GPUs with Slurm](Using_GPUs_with_SLURM.md) for general information on using GPUs on our systems.

This is a job script for mdrun using 4 OpenMP threads and one GPU:

**File :** gpu\_gromacs\_job.sh
...
- [![warning](/mediawiki/images/c/c2/Warning_sign_16px.png)](FileWarning_sign_16pxpng.md) GROMACS versions 2020.0 up to and including 2021.5 contain a bug when used on GPUs of Volta or newer generations (i.e. V100, T4 and A100) with `mdrun` option `-update gpu` that could have perturbed the virial calculation and, in turn, led to incorrect pressure coupling. The GROMACS developers state in the 2021.6 Release Notes:[3]
...
- Our clusters have differently configured GPU nodes.  
  On the page [Using GPUs with Slurm#Available GPUs](Using_GPUs_with_SLURM.md#Available_GPUs) you can find more information about the different node configurations (GPU models and number of GPUs and CPUs per node).
...
**File :** gpu\_gromacs\_job\_multidir.sh
...
The second solution is to use a [MIG](Multi-Instance_GPU.md) instance (a fraction of a GPU) rather than a full GPU. This is the preferred solution if you have a single simulation or if your simulations are dissimilar, for instance:
...
[![warning](/mediawiki/images/c/c2/Warning_sign_16px.png)](FileWarning_sign_16pxpng.md) Note that [Hyper-Q / MPS](Hyper-Q___MPS.md) should never be used with GROMACS. The built-in `-multidir` option achieves the same functionality more efficiently.
...
[the number of nodes you have access to](Job_scheduling.md#Percentage_of_the_nodes_you_have_access_to)
...
dedicated to mdrun-performance[7]
...
GROMACS uses **Domain Decomposition**[8] (DD)
...
and either set the *OMP\_NUM\_THREADS* variable with:
...
[VMD](VMD.md) is a molecular visualization program for displaying, animating, and analyzing large biomolecular systems using 3-D graphics and built-in scripting. It can be used to visually inspect GROMACS trajectories and also offers a large number of built-in and external plugins for analysis.
It can also be used in command line mode.

## Using Python

[MDAnalysis](https://www.mdanalysis.org/) and [MDTraj](https://www.mdtraj.org/) are two [Python](Python.md) packages that we provide as [precompiled Python wheels](Wheels.md). They can read and write trajectory and coordinate files of *GROMACS* (*TRR* and *XTC*) and many other MD packages and also include a variety of commonly used analysis functions. *MDAnalysis* can also read topology information from *GROMACS* *TPR* files, though often not those created by the latest versions of *GROMACS*.
...
StdEnv/2023StdEnv/2020StdEnv/2018 and StdEnv/2016
...
These [software environments](Standard_software_environments.md) are no longer supported.
...
Colvars[11] is a software module for molecular simulation programs, which
...
CP2K[17] is a quantum chemistry and solid-state physics software package.
Since version 2022 GROMACS can be compiled with CP2K-support[18] to enable Hybrid Quantum-Classical simulations (QM/MM)[19].
...
GROMACS-LS[20] and the MDStress library enable the calculation of local stress fields from molecular dynamics simulations.
The MDStress library is included in the GROMACS-LS module.

Please refer to manual for GROMACS-LS at: [Local\_stress.pdf](https://vanegaslab.org/files/Local_stress.pdf) and the publications listed therein for information about the method and how to use it.
...
GROMACS-RAMD is a fork of GROMACS that implements the *Random Acceleration Molecular Dynamics* (RAMD) method.[21]
...
Information on RAMD-specific MDP options[22]
can be found on the [GROMACS-RAMD GitHub page](https://github.com/HITS-MCM/gromacs-ramd).
...
Please refer to the [GROMACS-SWAXS Documentation](https://cbjh.gitlab.io/gromacs-swaxs-docs/) for
...
G\_MMPBSA[24] is a tool that calculates components of binding energy using MM-PBSA method except the entropic term and energetic contribution of each residue to the binding using energy decomposition scheme.
...
For newer version of GROMACS consider using gmx\_MMPBSA[25] instead (see below).
...
Please be aware that G\_MMPBSA uses implicit solvents and there have been studies[26] that conclude that there are issues with the accuracy of these methods for calculating binding free energies.

## gmx\_MMPBSA

gmx\_MMPBSA[25]
is a tool based on [AMBER](AMBER.md)'s MMPBSA.py aiming to perform end-state free energy calculations with GROMACS files.

Other than the older G\_MMPBSA[24], which is only compatible with older versions of GROMACS,
gmx\_MMPBSA can be used with current versions of GROMACS and [AmberTools](AMBER.md#AmberTools_21).

Please be aware that gmx\_MMPBSA uses implicit solvents and there have been studies[26] that conclude that there are issues with the accuracy of these methods for calculating binding free energies.

### Submission scripts

This submission script installs and executes gmx\_MMPBSA in a temporary directory on the local disk of a compute node. All MPI tasks must be on one node. For multi-node submission install a virtual environment on a shared filesystem.

**File :** job\_gmx\_MMPBSA.sh
...
### Installing gmx\_MMPBSA into a virtualenv

gmx\_MMPBSA needs to be installed in a permanent directory if you intend to use interactive visualization.

#### Installing for gromacs/2024 (StdEnv/2023)

```
[name@server ~]$ module load  StdEnv/2023  ambertools/23.5  gromacs/2024.4  qt/5.15.11
[name@server ~]$ virtualenv venv-gmxMMPBSA
[name@server ~]$ source venv-gmxMMPBSA/bin/activate
[name@server ~]$ pip install --no-index gmx_MMPBSA==1.6.3
...
```
[name@server ~]$ git clone https://github.com/Valdes-Tresanco-MS/gmx_MMPBSA
[name@server ~]$ cd gmx_MMPBSA/examples/Protein_DNA
[name@server ~]$ gmx_MMPBSA -O -i mmpbsa.in -cs com.tpr -ct com_traj.xtc -ci index.ndx -cg 3 4 -cp topol.top \ 
...
[name@server ~]$ module load StdEnv/2020 gcc/9.3.0 python/3.8  gromacs/2021.4
[name@server ~]$ module load ambertools/21
[name@server ~]$ virtualenv venv_gmxMMPBSA
[name@server ~]$ source venv_gmxMMPBSA/bin/activate
[name@server ~]$ pip install -U pip
[name@server ~]$ pip install --no-index numpy==1.22.2 seaborn==0.13.1 gmx_MMPBSA==1.5.0.3 ParmEd==3.4.4
...
```
[name@server ~]$ module load qt/5.15.2
...
```
[name@server ~]$ gmx_MMPBSA -h
[name@server ~]$ gmx_MMPBSA_test -ng -n 4

```

Fortunately, running the self-test is very quick, therefore it's permissible to run them on the login node.

Later when using gmx\_MMPBSA in a job you need to load the modules and activate the virtualenv as follows:

```
[name@server ~]$ module purge
[name@server ~]$ module load StdEnv/2020 gcc/9.3.0 python/3.8 ambertools/21 gromacs/2021.4 qt/5.15.2
[name@server ~]$ source venv_gmxMMPBSA/bin/activate
...
8. ↑ 8.0 8.1  [GROMACS User-Guide: Performance background information](http://manual.gromacs.org/documentation/current/user-guide/mdrun-performance.html#gromacs-background-information)
9. ↑ [GROMACS User-Guide: GPU accelerated calculation of PME](http://manual.gromacs.org/documentation/2018.1/user-guide/mdrun-performance.html#gpu-accelerated-calculation-of-pme)
10. ↑ [PLUMED Home](http://www.plumed.org/home)
11. ↑ [Colvars Home](https://colvars.github.io/)
12. ↑ [GROMACS 2024 Major Release Highlights](https://manual.gromacs.org/2024.1/release-notes/2024/major/highlights.html)
13. ↑ [Collective Variable simulations with the Colvars module (GROMACS Reference manual)](https://manual.gromacs.org/current/reference-manual/special/colvars.html)
14. ↑ [Colvars .mdp Options (GROMACS User guide)](https://manual.gromacs.org/current/user-guide/mdp-options.html#collective-variables-colvars-module)
15. ↑ [Colvars Reference manual for GROMACS](https://colvars.github.io/colvars-refman-gromacs/colvars-refman-gromacs.html)
16. ↑ [Fiorin et al. **2013**, *Using collective variables to drive molecular dynamics simulations.*](http://dx.doi.org/10.1080/00268976.2013.813594)
17. ↑ [CP2K Home](https://www.cp2k.org/)
18. ↑ [Building GROMACS with CP2K QM/MM support](https://manual.gromacs.org/documentation/current/install-guide/index.html#building-with-cp2k-qm-mm-support)
19. ↑ [QM/MM with CP2K in the GROMACS Reference manual](https://manual.gromacs.org/documentation/current/reference-manual/special/qmmm.html)
20. ↑ [GROMACS-LS and MDStress library](https://vanegaslab.org/software)
21. ↑ [Information on the RAMD method](https://kbbox.h-its.org/toolbox/methods/molecular-simulation/random-acceleration-molecular-dynamics-ramd/)
22. ↑ [RAMD-specific MDP options](https://github.com/HITS-MCM/gromacs-ramd#usage)
23. ↑ [GROMACS-SWAXS Home](https://biophys.uni-saarland.de/software/gromacs-swaxs/)
24. ↑ 24.0 24.1 [G\_MMPBSA Homepage](http://rashmikumari.github.io/g_mmpbsa/)
25. ↑ 25.0 25.1 [gmx\_MMPBSA Homepage](https://valdes-tresanco-ms.github.io/gmx_MMPBSA/dev/getting-started/)
26. ↑ 26.0 26.1 [Comparison of Implicit and Explicit Solvent Models for the Calculation of Solvation Free Energy in Organic Solvents](http://pubs.acs.org/doi/abs/10.1021/acs.jctc.7b00169)