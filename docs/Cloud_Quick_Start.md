...
- English
- [français](Cloud_Quick_Start_fr.md)
...
[![](/mediawiki/images/thumb/b/b3/Import_key_pair_3.png/500px-Import_key_pair_3.png)](FileImport_key_pair_3png.md)
...
1. *Details*
   ...
   - *Count:* This indicates the number of virtual machines to create. Unless you have specifically planned for multiple machines leave this set at 1.[![](/mediawiki/images/thumb/b/bc/Source_tab.png/500px-Source_tab.png)](FileSource_tabpng.md)
2. *Source*
   - *Select Boot Source:* Because it's your first VM, select *Image* as the boot source. For information about other options see [Booting from a volume](Working_with_volumes.md#Booting_from_a_volume).
...
4. *Networks:* Do not change this unless required. On Arbutus, select your project network by default (usually starting with *def-project-name*).[![](/mediawiki/images/thumb/e/e9/Security_groups.png/500px-Security_groups.png)](FileSecurity_groupspng.md)
5. *Network Ports:* Do not change this now.
6. *Security Groups:* The default security group should be in the *Allocated* list. If it is not, move it from *Available* to *Allocated* using the upwards arrow located on the far right of the group's row. For more information see [Security Groups](OpenStack.md#Security_Groups).[![](/mediawiki/images/thumb/1/15/Key_pair_tab.png/500px-Key_pair_tab.png)](FileKey_pair_tabpng.md)
...
8. *Configuration:* Do not change this now. For more information on customization scripts see [Using CloudInit](OpenStack_VM_Setups.md#Using_CloudInit).
...
[![](/mediawiki/images/thumb/e/eb/Manage-Floating-IP-Associations-Form.png/400px-Manage-Floating-IP-Associations-Form.png)](FileManage-Floating-IP-Associations-Formpng.md)

Manage Floating IP (Click for larger image)

[![](/mediawiki/images/thumb/8/81/Add-Rule-Form.png/400px-Add-Rule-Form.png)](FileAdd-Rule-Formpng.md)
...
3. Important notes
   - **Do not remove the default security rules** as this will affect the ability of your VM to function properly (see [Security Groups](OpenStack.md#Security_Groups)).
...
[![](/mediawiki/images/thumb/4/42/MobaXterm_basic.png/400px-MobaXterm_basic.png)](FileMobaXterm_basicpng.md)

Creating an SSH session (Click for larger image)

If you want to use a Windows computer to connect to your VM, you will need to have an interface application to handle the SSH connection. We recommend **MobaXTerm**, and will show the instructions for connecting with MobaXTerm below. If you want to connect using PuTTY instead, see [Connecting with PuTTY](Connecting_with_PuTTY.md).

[![](/mediawiki/images/thumb/8/81/MobaXterm_ssh_key.png/400px-MobaXterm_ssh_key.png)](FileMobaXterm_ssh_keypng.md)
...
[![](/mediawiki/images/thumb/b/ba/MobaXterm_connected.png/400px-MobaXterm_connected.png)](FileMobaXterm_connectedpng.md)
...
- See [configuring a data or web server](Setup_a_data_or_web_server.md)
- Learn more about working with [OpenStack](OpenStack.md)
- [Cloud Technical Glossary](Cloud_Technical_Glossary.md)
- [Automating VM creation](OpenStack_VM_Setups.md)
...
[![](/mediawiki/images/thumb/b/b0/Create-Key-Pair-Form.png/400px-Create-Key-Pair-Form.png)](FileCreate-Key-Pair-Formpng.md)
...
3. Click on [![](/mediawiki/images/8/81/Create-Key-Pair-Button.png)](FileCreate-Key-Pair-Buttonpng.md); the *Create Key Pair* window is displayed.
4. Give your key pair a name.
5. Click *Create Key Pair* button.
6. Save the <key name>.pem file on your local drive.

If you would like to use an existing key pair with your Windows VM see the [comments on key pairs](Cloud_Quick_Start.md#Comments_on_key_pairs) below.

## Launching a VM

[![](/mediawiki/images/thumb/b/b4/Windows-launch-instance.png/400px-Windows-launch-instance.png)](FileWindows-launch-instancepng.md)

Launch Instance (Click for larger image)

To create a virtual machine, click on the *Instances* menu item on the left, then click on [![](/mediawiki/images/9/94/Launch-Instance-Button.png)](FileLaunch-Instance-Buttonpng.md)
...
- *Details* tab
  ...
  - *Flavor*: The flavor defines virtual machine hardware specifications; choose the 'p2-3gb' flavor.
    ...
    The Windows image is quite large and requires a large bootable drive. C-flavors, as described [here](Virtual_machine_flavors.md), only have root drives of 20 GB, choosing a "p" flavor allows for larger root volumes. The smallest "p" flavor has 1.5 GB of RAM and from experience this is too little to run Windows well. Choosing a slightly larger flavor, such as "p2-3gb", improves the performance of the VM.
...
- *Access & Security* tab
  - *Key pair*: Select your SSH key pair.
    ...
    If you have only one, it is selected by default. If you do not have a key pair, please see [above](Cloud_Quick_Start.md#SSH_key_pair).
...
[![](/mediawiki/images/thumb/4/4d/Windows-VM-Settings.png/400px-Windows-VM-Settings.png)](FileWindows-VM-Settingspng.md)
...
[![](/mediawiki/images/thumb/f/fd/Add-Rule-Form-RDP.png/400px-Add-Rule-Form-RDP.png)](FileAdd-Rule-Form-RDPpng.md)
...
1. On the *Security Groups* tab, select *Access & Security*; on the default row, click [![](/mediawiki/images/2/20/Manage-Rules-Button.png)](FileManage-Rules-Buttonpng.md)
2. On the next screen, click [![](/mediawiki/images/f/fe/Add-Rule-Button.png)](FileAdd-Rule-Buttonpng.md)
...
[![](/mediawiki/images/thumb/1/12/Retrieve-instance-password.png/400px-Retrieve-instance-password.png)](FileRetrieve-instance-passwordpng.md)

Retrieving Windows instance password (Click for larger image)

[![](/mediawiki/images/thumb/a/a3/Remote-Desktop-Connection-windows.png/400px-Remote-Desktop-Connection-windows.png)](FileRemote-Desktop-Connection-windowspng.md)

Remote desktop client in Windows (Click for larger image)

[![](/mediawiki/images/thumb/9/93/Remmina-Ubuntu.png/400px-Remmina-Ubuntu.png)](FileRemmina-Ubuntupng.md)
...
There are a couple different formats for key files and you can also choose to protect your private keys with passphrases or not. In order to be able to decrypt the Windows VM password your private key must be in OpenSSH format and not have a passphrase. If you created your key pair with OpenStack and downloaded the `.pem` key file it will already be in the correct format. If you used the [`ssh-keygen` command](Using_SSH_keys_in_Linux.md) to create your key pair and didn't specify a passphrase it will also likely be in the correct format. For more general information about key pairs see the [SSH Keys](SSH_Keys.md) page.
