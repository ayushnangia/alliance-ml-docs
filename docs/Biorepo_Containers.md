...
2. A workflow that requires [Conda](Conda.md) and can not easily be ported to use [pip](Python.md#Creating_and_using_a_virtual_environment) and our [modules](Available_software.md). A container can be used when the desired tools must be or are much more easily installed with Conda or a similar package manager.
