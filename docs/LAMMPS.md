...
- English
- [français](LAMMPS_fr.md)

*Parent page: [Biomolecular simulation](Biomolecular_simulation.md)*
...
- Project web site: [http://lammps.sandia.gov/](http://lammps.sandia.gov/)
...
LAMMPS is parallelized with [MPI](MPI.md) and [OpenMP](OpenMP.md), and can run on [GPUs](Using_GPUs_with_SLURM.md).

# Force fields

All supported force fields are listed on the [package web site](https://lammps.sandia.gov/doc/Intro_features.html#ff),
...
To see which versions of LAMMPS are installed on our systems, run `module spider lammps`. See [Using modules](Utiliser_des_modules.md) for more about `module` subcommands.
...
In order to use the GPU-enabled version, load the [CUDA](CUDA.md) module before loading the LAMMPS module:
