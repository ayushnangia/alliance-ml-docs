...
- English
- [français](Using_node-local_storage_fr.md)

When [Slurm](Scheduler.md) starts a job, it creates a temporary directory on each node assigned to the job.
...
is almost always faster than I/O to a [network storage](Filesystem.md) (/project, /scratch, or /home).
...
[creating virtual environments inside your jobs](Python.md#Creating_virtual_environments_inside_of_your_jobs)
using `$SLURM_TMPDIR`.
...
- write [checkpoints](Points_de_contrôle.md) to network storage, not to `$SLURM_TMPDIR`;
...
At **[Trillium](Trillium.md)**, $SLURM\_TMPDIR is implemented as *RAMdisk*,
...
If your job reserves [whole nodes](Advanced_MPI_scheduling.md#Whole_nodes),
then you can reasonably assume that this much space is available to you in $SLURM\_TMPDIR on each node.
...
See *Node characteristics* at the appropriate cluster's page ([Fir](Fir.md), [Narval](Narval.md), [Nibi](Nibi.md), [Rorqual](Rorqual.md)) for guidance.