...
- English
- [français](Managing_Slurm_accounts_fr.md)

Each job submitted to the [job scheduler Slurm](Scheduler.md) has an associated
Resource Allocation Project (RAP) which is selected with the `--account` option to `sbatch`.
The scheduling priority of the job will be determined by the target share of the account relative to the account's recent usage, as described at [Job scheduling policies](Job_scheduling.md).
