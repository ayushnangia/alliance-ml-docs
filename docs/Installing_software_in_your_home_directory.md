...
- English
- [français](Installing_software_in_your_home_directory_fr.md)
...
If other errors arise, contact support. For more information see [Make](Make.md), [Autotools](Autotools.md), and [CMake](CMake.md).
...
The core of this [software stack](Standard_software_environments.md) is provided by the `gentoo/2023` module, which is loaded by default. This stack, internally managed using the Gentoo package manager, is located at `/cvmfs/soft.computecanada.ca/gentoo/2023/x86-64-v3/usr`. The environment variable `$EBROOTGENTOO` should be used to refer to this path.
...
then you should start an [interactive job](Scheduler.md#Interactive_jobs) on a host with the hardware you need, and compile from there.