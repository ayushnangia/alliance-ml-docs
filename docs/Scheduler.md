...
will submit the above job script with a time limit of 30 minutes. The acceptable time formats include "minutes", "minutes:seconds", "hours:minutes:seconds", "days-hours", "days-hours:minutes" and "days-hours:minutes:seconds". Please note that the time limit will strongly affect how quickly the job is started, since longer jobs are [eligible to run on fewer nodes](Job_scheduling.md).

Please be cautious if you use a script to submit multiple Slurm jobs in a short time. Submitting thousands of jobs at a time can cause Slurm to become [unresponsive](Frequently_Asked_Questions.md#sbatch:_error:_Batch_job_submission_failed:_Socket_timed_out_on_send/recv_operation) to other users. Consider using an array job instead, or use `sleep` to space out calls to `sbatch` by one second or more.
...
Every job must have an associated account name corresponding to a [Resource Allocation Project](Frequently_Asked_Questions_about_the_CCDB.md#What_is_a_RAP.3F) (RAP). If you are a member of only one account, the scheduler will automatically associate your jobs with that account.
...
[![](/mediawiki/images/0/01/Find-group-name-EN.png)](FileFind-group-name-ENpng.md)
...
A serial job is a job which only requests a single core. It is the simplest type of job. The "simple\_job.sh" which appears above in Use sbatch to submit jobs is an example.
...
Our [Machine Learning tutorial](Tutoriel_Apprentissage_machine.md) covers [resubmitting for long machine learning jobs](Tutoriel_Apprentissage_machine.md#Checkpointing_a_long-running_job).
...
Beluga, Fir, Narval, Nibi, RorqualTrillium
...
See [Trillium specific restrictions](Trillium_Quickstart.md#Trillium_specific_restrictions).
...
- For a discussion of how job priority is determined and how things like time limits may affect the scheduling of your jobs, see [Job scheduling policies](Job_scheduling.md).
