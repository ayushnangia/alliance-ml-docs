...
- English
- [français](RAC_transition_FAQ_fr.md)
...
- If an allocation has moved from one site to another, users are expected to transfer the data by themselves (via globus, scp, rsync, *etc.*; see [Transferring data](Data_transfer.md)). For large amounts of data (*e.g.*, 200TB or more) please [contact support](Technical_Support.md) for advice or assistance on managing the transfer.
