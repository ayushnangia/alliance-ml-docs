...
The [Genome Analysis Toolkit (GATK)](https://bio.tools/gatk) is a set of bioinformatic tools for
...
It is a leading tool in variant discovery and [best practices](https://gatk.broadinstitute.org/hc/en-us/sections/360007226651-Best-Practices-Workflows)
...
the [`module` command](https://docs.computecanada.ca/wiki/Utiliser_des_modules/en)
...
For the commands that do use Spark, you can request multiple CPUs. **NOTE:** Please provide the exact number of CPUs to the `spark` command. For example if you requested 10 CPUs, use `--spark-master local[10]` instead of `--spark-master local[*]`. If you want to use multiple nodes to scale the Spark cluster, you have to first [deploy a SPARK cluster](Spark.md) and then set the appropriate variables in the GATK command.
...
A Docker image of GATK can be found [here](https://hub.docker.com/r/broadinstitute/gatk) and other versions are available on this [page](https://hub.docker.com/r/broadinstitute/gatk/tags). You will need first to build an Apptainer image from the Docker image;
...
In your [SBATCH](Scheduler.md) script, you should use something like this:
...
1. ↑  [https://gatk.broadinstitute.org/hc/en-us/articles/360035890591-](https://gatk.broadinstitute.org/hc/en-us/articles/360035890591-)