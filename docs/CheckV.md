...
You will find all information about available software here [Available software](Available_software.md). In short, some of the softwares are available by loading the appropriate module.
...
Python modules are provided as binary wheels [Available Python wheels](Wheels.md).
...
First steps would be to have a look at the documentation of the software. You can easily find the software development page that is often based on a github repository and follow the installation steps. Note that you **cannot** use Conda environment on the clusters [Anaconda](Conda.md). We have a wiki page that explains how you can install it locally in your account [Installing software in your home directory](Installing_software_in_your_home_directory.md) or you can email the [Technical support](Technical_Support.md) to get some help to either install it in your account or in the clusters.
...
```
[name@server ~]$ module load gcc hmmer/3.3.2 prodigal-gv/2.6.3 diamond/2.0.4 python/3.10
...
```
[name@server ~]$ virtualenv ~/CheckV_env
[name@server ~]$ source ~/CheckV_env/bin/activate
...
```
[name@server ~]$ pip install --no-index checkv
...
```
[name@server ~]$ pip install checkv
...
```
[name@server ~]$ python -c 'import checkv'
...
```
[name@server ~]$ pip freeze > checkv-1.0.1-requirements.txt
...
```
[name@server ~]$ checkv download_database $SCRATCH/
...
```
[name@server ~]$ checkv update_database /path/to/checkv-db /path/to/updated-checkv-db genomes.fna

```

Some users may wish to download a specific database version. See [[[1]](https://portal.nersc.gov/CheckV/)] for an archive of all previous database versions. If you go this route then you'll need to build the DIAMOND database manually:

```
[name@server ~]$ wget https://portal.nersc.gov/CheckV/checkv-db-archived-version.tar.gz

```

**2. Download a sequence test**
Some software will give access to a data set for you to test the software. You can look if anything is available on the web or the github repository. For CheckV, the data set is available here [[2]](https://bitbucket.org/berkeleylab/checkv/src/master/test/). You can download it with this command:

```
[name@server ~]$ wget https://bitbucket.org/berkeleylab/checkv/raw/3f185b5841e8c109848cd0b001df7117fe795c50/test/test_sequences.fna
...
If you need to refresh your SBATCH parameter knowledge we recommend having a look at the [Slurm SBATCH command page](https://slurm.schedmd.com/sbatch.html) and the [Running jobs](Scheduler.md) wiki page.

To learn more about interactive job you can have a look at the wiki page [Running jobs#Interactive job](Scheduler.md#Interactive_job).
...
```
[name@server ~]$ checkv end_to_end input_file.fna output_directory -t 16

```

In this case, you should be intrigued in finding what the `-t` parameter is doing. To have access to the help menu for end\_to\_end program:

```
[name@server ~]$ checkv end_to_end --help
...
```
[name@server ~]$ salloc --time=1:0:0 --mem=2G --cpus-per-task=16 --account=def-someuser
...
```
[name@server ~]$ checkv end_to_end test_sequences.fna $SCRATCH/checkv_output -t ${SLURM_CPUS_PER_TASK:-1} -d $SCRATCH/checkv-db-v1.5
...
To have the whole picture about commands involved in job monitoring please have a look at this wiki page: [Running\_jobs#Monitoring\_jobs Monitoring jobs].
