...
Alternatively, you can use the JAVA\_TOOL\_OPTIONS environment variable to set the run-time options rather that passing them on the command line. This is especially convenient if you launch multiple Java calls, or call a Java program from another Java program. Here is an example how to do it:
...
When your Java program is run, it will produce a diagnostic message like this one "Picked up JAVA\_TOOL\_OPTIONS", verifying that the options have been picked up.
