...
Try [JupyterLab](JupyterLab.md) first
...
[![](/mediawiki/images/thumb/4/4f/Matlab-vnc.png/400px-Matlab-vnc.png)](FileMatlab-vncpng.md)
...
Before starting your VNC server, log into a cluster (such as Nibi) and create a compute node allocation using the `salloc` command (24hr time limit applies). For example, to request an [interactive job](Scheduler.md#Interactive_jobs) using 4 CPUs and 16GB of memory you could use the command:
...
If you are on a Mac or Windows desktop (not a linux distro) then instead of running the `vncviewer` from the command line, you may click the *TigerVNC Viewer* application icon and enter your **localhost:port** information as shown here: [![thumbMac Tiger VNC Viewer Connection Details Dialogue Box](/mediawiki/images/thumb/e/ec/VNCviewerConnect3.png/400px-VNCviewerConnect3.png)](FileVNCviewerConnect3png.md). As a side note, the default VNC port assumed by *TigerVNC Viewer* is 5900, therefore if you specified 5900 as the local port to be forwarded when you started your SSH tunnel, then you could simply specify **localhost**. Windows users however may find they cannot set up an SSH tunnel on local port 5900 in the first place.
