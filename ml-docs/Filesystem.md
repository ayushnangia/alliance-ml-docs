...
- English
- [français](Storage_and_file_management_fr.md)
...
- **NEARLINE**: Nearline is a tape-based filesystem intended for inactive data. Datasets which you do not expect to access for months are good candidates to be stored in /nearline. For more information, see [Using nearline storage](Nearline.md).
- **SLURM\_TMPDIR**: While a job is running, the environment variable `$SLURM_TMPDIR` holds a unique path to a temporary folder on a fast, local filesystem on each compute node allocated to the job. When the job ends, the directory and its contents are deleted, so `$SLURM_TMPDIR` should be used for temporary files that are only needed for the duration of the job. Its advantage, compared to the other networked filesystem types above, is increased performance due to the filesystem being local to the compute node. It is especially well-suited for large collections of small files (for example, smaller than a few megabytes per file). Note that this filesystem is shared between all jobs running on the node, and that the available space depends on the compute node type. A more detailed discussion of using `$SLURM_TMPDIR` is available at [this page](Using_SLURM_TMPDIR.md).
...
- Regularly clean up your data in the scratch and project spaces, because those filesystems are used for huge data collections.
  ...
  - If possible, avoid using `*` and `/` characters in your `rm` commands.
    ...
    - If the directory has a [Makefile](Make.md), it may use `*` and `/` in `rm` commands, but these commands have to be well tested.
...
- As far as possible, use scratch and local storage for temporary files. For local storage you can use the temporary directory created by the [job scheduler](Scheduler.md) for this, named `$SLURM_TMPDIR`.
- If your program must search within a file, it is fastest to do it by first reading it completely before searching.
- If you no longer use certain files but they must be retained, [archive and compress](Archiving_and_compressing_files.md) them, and if possible move them to an alternative location like [nearline](Nearline.md).
...
FirNibiNarval and RorqualTrilliumKillarneyTamIAVulcan
...
| Scratch Space | 20 TB hard / 1TB soft and 1M files per user [ 2 ] | No | No | No | Yes | Yes |
...
1. ↑ This quota is fixed and cannot be changed.
2. ↑ An 1 TB soft quota on scratch applies to each researcher. This soft quota can be exceeded for up to 60 days after which no additional files may be written to scratch. Files may be written again once the researcher has removed or deleted enough files to bring their total scratch use under 1 TB. See [Scratch purging policy](Scratch_purging_policy.md) for more information.
3. ↑ Project space can be increased to 40 TB per group by a RAS request, subject to the limitations that the minimum project space per quota cannot be less than 1 TB and the sum over all four general-purpose clusters cannot exceed 43 TB. The group's sponsoring PI should write to [technical support](Technical_Support.md) to make the request.
...
- [Transferring data](Data_transfer.md)
