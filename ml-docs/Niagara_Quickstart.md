...
- English
- [français](Niagara_Quickstart_fr.md)

# Specifications

The Niagara cluster is a large cluster of 1548 Lenovo SD350 servers each with 40 Intel "Skylake" cores at 2.4 GHz.
The peak performance of the cluster is 3.02 PFlops delivered / 4.75 PFlops theoretical. It is the 53rd fastest supercomputer on the [TOP500 list of June 2018](https://www.top500.org/list/2018/06/?page=1).
...
More detailed hardware characteristics of the Niagara supercomputer can be found [on this page](Niagara.md).
...
Please read this document carefully. The [FAQ](https://docs.scinet.utoronto.ca/index.php/FAQ) is also a useful resource. If at any time you require assistance, or if something is unclear, please do not hesitate to contact us.
...
the [Linux introduction](Linux_introduction.md) or to attend a local "Linux Shell" workshop.

As with all SciNet and Alliance compute systems, access to Niagara is done via ssh (secure shell) only. As of January 22 2022, authentication is only allowed via SSH keys. Please refer to [this page](SSH_Keys.md) to generate your SSH key pair and make sure you use them securely.

Open a terminal window (e.g. [PuTTY](Connecting_with_PuTTY.md) on Windows or [MobaXTerm](Connecting_with_MobaXTerm.md)), then ssh into the Niagara login nodes with your CC credentials:
...
You should familiarize yourself with the [various file systems](Data_Management_at_Niagara.md#Purpose_of_each_file_system), what purpose they serve, and how to properly use them. This table summarizes the various file systems. See the [Data management at Niagara](Data_Management_at_Niagara.md) page for more details.
...
Details of how to use the datamover nodes can be found on the [Data management at Niagara](Data_Management_at_Niagara.md#Moving_data) page.
...
Other than essentials, all installed software is made available [using module commands](Utiliser_des_modules.md). These modules set environment variables (PATH, etc.), allowing multiple, conflicting versions of a given package to be available. A detailed explanation of the module system can be [found on the modules page](Utiliser_des_modules.md).
...
Along with modifying common environment variables, such as PATH, and LD\_LIBRARY\_PATH, these modules also create a SCINET\_MODULENAME\_ROOT environment variable, which can be used to access commonly needed software directories, such as /include and /lib.
...
1. A [Niagara software stack](Modules_specific_to_Niagara.md) tuned and compiled for this machine. This stack is available by default, but if not, can be reloaded with

   `module load NiaEnv`
2. The standard [Alliance software stack](Available_software.md) which is available on Alliance's other clusters (including [Graham](Graham.md), [Cedar](Cedar.md), [Narval](Narval.md), and [Beluga](Beluga.md)):
...
Please visit the [Python](Python.md) or [R](R.md) page for details on using these tools. For information on running MATLAB applications on Niagara, visit [this page](MATLAB.md).
...
- That means no [MATLAB](MATLAB.md), Gaussian, IDL,
- Open source alternatives like Octave, [Python](Python.md), and [R](R.md) are available.
...
- The optimization flags -O3 -xHost allow the Intel compiler to use instructions specific to the architecture CPU that is present (instead of for more generic x86\_64 CPUs).
...
- Short tests that do not fit on a login node, or for which you need a dedicated node, request an interactive debug job with the debug command:
  ...
  If you need to use graphics while testing your code through salloc, e.g. when using a debugger such as DDT or DDD, you have the following options, please visit the
  [Testing with graphics](Testing_With_Graphics.md) page.
...
- Compute nodes have no internet access.
  ...
  [Move your data](Data_Management_at_Niagara.md#Moving_data) to Niagara before you submit your job.
...
Further instructions for monitoring your jobs can be found on the [Slurm page](https://docs.scinet.utoronto.ca/index.php/Slurm#Monitoring_jobs). The [my.SciNet](https://my.scinet.utoronto.ca) site is also a very useful tool for monitoring your current and past usage.
...
- SciNet: [https://www.scinet.utoronto.ca](https://www.scinet.utoronto.ca)
- Niagara: [Niagara wiki page](Niagara.md)
- System Status: [https://docs.scinet.utoronto.ca/index.php/Main\_Page](https://docs.scinet.utoronto.ca/index.php/Main_Page)
- Training: [https://support.scinet.utoronto.ca/education](https://support.scinet.utoronto.ca/education)

**Support**
Contact our [Technical support](Technical_Support.md)