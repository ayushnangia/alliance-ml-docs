...
- English
- [français](Handling_large_collections_of_files_fr.md)

In certain domains, notably [AI and Machine Learning](AI_and_Machine_Learning.md), it is common to have to manage very large collections of files, meaning hundreds of thousands or more. The individual files may be fairly small, e.g. less than a few hundred kilobytes. In these cases, a problem arises due to [filesystem quotas](Filesystem.md#Filesystem_quotas_and_policies) on our clusters that limit the number of filesystem objects. Very large numbers of files, particularly small ones, create significant problems for the performance of these shared filesystems as well as the automated backup of the home and project spaces.
...
Disk archive utility, conceived of as a significant modernization of the venerable [tar](Tar.md) tool. For more information, see [Dar](Dar.md).
...
The SQLite executable is called `sqlite3`. It is available via the `nixpkgs` [module](Utiliser_des_modules.md), which is loaded by default on our systems.
